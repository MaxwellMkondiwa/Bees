$title simulate the effect of winter mortality changes on Bee market


$include c-Sim_No_forage

set iters/ iter1*iter101/;
set iter(iters) ;
iter(iters)=yes;


set winter(period)/  Oct2, Nov1, Nov2 ,  Dec1  ,   Dec2  ,   Jan1 ,  Jan2, Feb1 /;


set comp components in the objective function /percent, cs,ps, wel, shippingC, splitC, feedingC, extractC,PoliC, total,
                                                 shadowprice, honey_p, honey_q, honey_rev, poli_rev, under_dem/
...
shocks(iters)= ord(iters)*0.01-0.51 ;
display shocks;
option solprint=off;
option limrow=0;
option limcol=0;

scalar c changes of the almond acres /1/;
...
loop(iter,
         ...
         V(winter, location)= (1+shocks(iter))**(1/8)* Vsave(winter, location);
         V(period, location)$ (Vsave(period, location)=0)=0;
...
$ondotl
         ...
         report_Honey(t,iter, 'price')                                                           = [(HONEYSALES2.L(t)$(HONEYSALES2.l(t)>0)/honeyquantity1(t))**(1./epsilon) ]* honeyprice1(t) + ([min( k, (1/k)**(1/epsilon))]* honeyprice1(t) )$(HONEYSALES1.l(t)>0);
         ...
         report_welfare('honey_q',t, iter)
                    $(HONEYSALES1(t)+ HONEYSALES2(t)>0.1) =    1000*(HONEYSALES1(t)+ HONEYSALES2(t)) ;
         ...
         Decomp('honey_q','before','all',t)$(HONEYSALES1(t)+ HONEYSALES2(t)>0.1)   = 1000*(HONEYSALES1(t)+ HONEYSALES2(t))                   ;
...
* resolve the model
         ...
         PollHives(Period,Location,t)$sum(MapCropPollSeason('almond', Location, Period), PollReq('almond',t))
                 = c+  sum(MapCropPollSeason('almond', Location, Period), PollReq('almond',t)) ;
...
* calculate the marginal effect and save it
         Decomp(comp,'diff',bb,t)                =       (Decomp(comp,'after',bb,t)- Decomp(comp,'before',bb,t))/c;
...
execute_unload "Mortality_joint_%yyear1%_%yyear2%_%Vbound%.gdx" Vnew honeynew  modelstat modelstat1 report_poliprice report_decomp shocks
                                         report_honey  report_beel report_beeal report_splitl report_Bee_hBee     report_welfare

execute 'gdxxrw.exe Mortality_joint_%yyear1%_%yyear2%_%Vbound%.gdx par=Vnew rng=V!A1 par=honeynew rng=HoneyRate!A1  par=shocks rng=shocks!A1   par=modelstat rng=modelstat!A1  par=modelstat1 rng=modelstat!A5'
execute 'gdxxrw.exe Mortality_joint_%yyear1%_%yyear2%_%Vbound%.gdx par=report_poliprice rng=PollPrice!A1 cdim=2 par=report_decomp rng=decomposition!A1 cdim=2  par=report_honey rng=honeyPQ!A1 cdim=1 par=report_Bee_hBee rng=almond_Bees!A1 cdim=3 par=report_welfare rng=welfare!A1 cdim=2'
