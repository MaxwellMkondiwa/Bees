$title simulate the effect of winter mortality changes on Bee market with forage scarcity adjustment
...
set iters used for almonad acrage scenarios/ iter1*iter31/;
set iter(iters) ;
iter(iters)=yes;

set winter(period)/ Oct2        ,         Nov1
...
parameter r_fratio(period, Flocation, iters)                  record the fratio
          ...
          vsave_shock(period, location)                       save the simulated v
          ...
          report_beeFl(period, Flocation,subt, iters)         save bee solution
          ...
          report_polihives(period, location,subt, iters)      report polination hives;
...
shocks(iters)= ord(iters)*0.02-0.32;
...
*calibrate the winter mortality rate
...
loop(iter,
         ...
         Vsave_shock(period, location)= Vsave(period, location);
         V(period, location)= Vsave(period, location);
         honey(Period,Location)= honeysave(Period,Location);
         Vsave_shock(winter, location)= (1+shocks(iter))**(1/8)* Vsave(winter, location);
         Vsave_shock(period, location)$ (Vsave(period, location)=0)=0;
         V(period, location)= Vsave_shock(period, location);
         beebase(period, Flocation,subt)= beebase_save(period, Flocation,subt);

         SOLVE TRANSPORT USING dNLP MINIMIZING Obj;
         beesave(period,Flocation,t)=0;
...
$ondotl
         report_BEEL(Period,subLocation,subt,iter)$(BEE(Period,subLocation,subt)>0.1)            = BEE(Period,subLocation,subt)                 ;

         report_beeFl(period, Flocation,subt, iter)$(BEEF(Period,FLocation,subt)>0.1)            = BEEF(Period,FLocation,subt)                 ;
         report_BEEAL(Period,sublocation,subt,iter)$(BEEA(Period,sublocation,subt)>0.1)          = BEEA(Period,sublocation,subt)                 ;
         report_SPLITL(Period,subLocation,subt,iter)$(SPLIT(Period,subLocation,subt)>0.1)        = SPLIT(Period,subLocation,subt)                 ;
...
* resolve the model
         ...
         PollHives(Period,Location,subt)$sum(MapCropPollSeason('almond', Location, Period), PollReq('almond',subt))
                         = 1+  sum(MapCropPollSeason('almond', Location, Period), PollReq('almond',subt))   ;
...
$ondotl
         ...
         Decomp('honey_q','after','all',subt)$(HONEYSALES1(subt)+ HONEYSALES2(subt)>0.1)                    = 1000*(HONEYSALES1(subt)+ HONEYSALES2(subt)  )                 ;
...
execute_unload "Mortality_forage_joint_%yyear1%_%yyear2%_%Vbound%_%betavalue%.gdx" Vnew honeynew   modelstat1 report_poliprice report_decomp     r_fratio    shocks
                                         report_honey  report_beel report_beeal report_splitl   report_honeyV report_V   report_Bee_hBee   report_welfare

execute 'gdxxrw.exe Mortality_forage_joint_%yyear1%_%yyear2%_%Vbound%_%betavalue%.gdx par=Vnew rng=V!A1 par=honeynew rng=HoneyRate!A1  par=shocks rng=shocks!A1   par=modelstat1 rng=modelstat1!A1'
execute 'gdxxrw.exe Mortality_forage_joint_%yyear1%_%yyear2%_%Vbound%_%betavalue%.gdx par=report_poliprice rng=PollPrice!A1 cdim=2 par=report_decomp rng=decomposition!A1 cdim=2  par=report_honey rng=honeyPQ!A1  par=report_Bee_hBee rng=almond_Bees!A1 cdim=3 par=report_welfare rng=welfare!A1 cdim=2 '
execute 'gdxxrw.exe Mortality_forage_joint_%yyear1%_%yyear2%_%Vbound%_%betavalue%.gdx par=report_honeyV rng=honeyV_adj!A1 par=report_V rng=V_adj!A1  par=r_fratio rng=r_fratio!A1'
